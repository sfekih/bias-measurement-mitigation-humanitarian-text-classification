...
class OneLabelModel(nn.Module):
    ...
    def forward(self, input_ids: torch.Tensor, attention_mask: torch.Tensor):
        ...
        """
        encoded = (
            input_ids.clone().detach().long(),
            attention_mask.clone().detach().long(),
        )
        preds = self.model(encoded)["classification"]
...
class MultiLabelClassificationExplainer:
    ...
    def __init__(self, model, attribution_type: str, device: str):
        """
        Args:
        ...
        assert (
            attribution_type in SUPPORTED_ATTRIBUTION_TYPES
        ), f"'attribution_type' parameter must be in {SUPPORTED_ATTRIBUTION_TYPES}"
        ...
        self.word_embeddings = (
            self.model.model.model.backbone.get_input_embeddings().to(device)
        )

        self.label2id = self.model.tagname_to_tagid_classification
    ...
    def _forward(
        ...
        preds = self.model(encoded)["classification"]  # logits

        return preds[:, self.tmp_selected_index]
    ...
    def _return_attributions_one_pass(self):
        ...
        if self.attribution_type == "Layer Integrated Gradients":
        ...
        elif self.attribution_type == "Layer LRP":
            one_label_model = OneLabelModel(self.model, self.tmp_selected_index)
            lig = LayerDeepLift(one_label_model, self.word_embeddings)
        ...
        elif self.attribution_type == "Layer GradientXActivation":
            one_label_model = OneLabelModel(self.model, self.tmp_selected_index)
            lig = LayerGradientXActivation(one_label_model, self.word_embeddings)
            ...
            raise ValueError(
                f"The parameter 'attribution_type' is wrong. Please choose one of {SUPPORTED_ATTRIBUTION_TYPES}."
            )
        ...
        for word, attribution in zip(self.reference_tokens, self.attributions_sum):
            word_attributions[word].append(float(attribution.cpu().data.numpy()))

        word_attributions = {k: np.mean(v) for k, v in word_attributions.items()}
    ...
    def __call__(
        ...
        Returns:
            dict: A dictionary of label to list of attributions. {tag: {word: float}}
        """
