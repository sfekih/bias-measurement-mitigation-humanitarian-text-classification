...
from debiasing_pipelines.counterfactual_embeddings_discrepency.counterfactual_embeddings_discrepency import (
    get_embeddings_results,
)
from debiasing_pipelines.counterfactual_explainability_discrepency.counterfactual_explainability_discrepency import (
    get_explaiability_results,
)
...
class TrainingPipeline:
    ...
    def get_counterfactual_explainability_discrepency_results(self):
        EXPLAINABILITY_RESULTS_DIR = os.path.join(self.RESULTS_DIR, "explainability")
        if not os.path.exists(EXPLAINABILITY_RESULTS_DIR):
            os.makedirs(EXPLAINABILITY_RESULTS_DIR)

            # self.output_explainability_results = {}

        for one_bias_type, one_bias_df in self.counterfactual_test_dfs.items():
            for one_explainability_method in [
                "Layer DeepLift",
                # "Layer GradientXActivation", # this method can also be used.
            ]:
                results_explainability_one_type = get_explaiability_results(
                    self.model,
                    one_bias_df,
                    one_explainability_method,
                    device=self.device,
                )

                file_name = f"{self.results_name}_{one_bias_type}_{one_explainability_method}_explainability.csv"

                results_explainability_one_type.to_csv(
                    os.path.join(
    ...
    def get_counterfactual_embeddings_discrepency_results(self):
        EMBEDDINGS_RESULTS_DIR = os.path.join(self.RESULTS_DIR, "embeddings")
        if not os.path.exists(EMBEDDINGS_RESULTS_DIR):
            os.makedirs(EMBEDDINGS_RESULTS_DIR)

        for one_bias_type, one_bias_df in self.counterfactual_test_dfs.items():
            results_embedding_one_bias_type = get_embeddings_results(
                self.model, one_bias_df.copy()
            ).drop(columns=["target_classification"])

            file_name = f"{self.results_name}_{one_bias_type}_embeddings.csv"

            results_embedding_one_bias_type.to_csv(
                os.path.join(
                    EMBEDDINGS_RESULTS_DIR,
                    file_name,
                ),
                index=None,
            )
